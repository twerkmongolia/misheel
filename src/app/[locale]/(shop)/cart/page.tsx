...
export default async function CartPage({
  ...
  const t = getDictionary(locale);
  const [cart, site] = await Promise.all([
    readCart(),
    getSiteContent(["shop"]),
  ]);
    ...
    return [
      {
        ...
        qty,
        requested: item.qty,
        capped: item.qty > row.variant.stock_qty,
      },
    ];
  });

  const subtotal = lines.reduce(
    (sum, line) => sum + line.variant.price * line.qty,
    0,
  );
  const itemCount = lines.reduce((sum, line) => sum + line.qty, 0);
  const shippingFee = Number(
    content(site.get("shop"), locale).shipping_fee ?? 5000,
  );
      ...
      <div className="grid items-start gap-8 lg:grid-cols-[1.7fr_1fr]">
        <ul className="card divide-y divide-line">
          {lines.map((line) => (
            <li
              key={line.variant.id}
              className="flex flex-wrap gap-5 p-5 sm:flex-nowrap"
            >
              <Link
