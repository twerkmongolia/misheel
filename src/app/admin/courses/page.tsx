...
export default async function AdminCoursesPage({
  ...
  const [{ data: access }, { data: enrollments }] = await Promise.all([
    ids.length
      ? supabase.from("course_access").select("*").in("course_id", ids)
      : { data: [] },
    ids.length
      ? supabase
            ...
            <tbody className="bg-black">
              {courses.map((course) => {
                ...
                return (
                  <tr
                    key={course.id}
                    className={course.is_active ? undefined : "opacity-55"}
                  >
                    <Td>
                      ...
                      <Sub>
                        {[
                          ...
                          course.starts_on
                            ? formatDate(
                                `${course.starts_on}T00:00:00+08:00`,
                                "mn",
                              )
                    ...
                    {!mode && (
                      <Td label="Горим">
                        <Badge tone={online ? "info" : "neutral"}>
  ...
  return (
    <>
      <form action={action} className="flex flex-col gap-5">
        ...
        <Field
          label="Горим"
          hint="Танхим = суудалтай, огноотой · Онлайн = Telegram бүлэг"
        >
        ...
        <Field label="Товч" hint="Жагсаалтын карт дээр гарна — нэг өгүүлбэр">
          <Textarea
            name="summary_mn"
            rows={2}
            defaultValue={course?.summary_mn ?? ""}
          />
        </Field>
        ...
        <Field
          label="Хуваарийн тэмдэглэл"
          hint="«Мяг, Пү 19:00» эсвэл «Өөрийн хэмнэлээр»"
        >
        ...
        <fieldset className="course-studio-only gap-5 border-t border-line pt-5">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="Байршил">
              <Select
                name="location_id"
                defaultValue={course?.location_id ?? ""}
              >
                <option value="">— сонгоогүй —</option>
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <Field
              label="Эхлэх өдөр"
              hint="Энэ өдөр өнгөрмөгц элсэлт өөрөө хаагдана"
            >
              <Input
                type="date"
                name="starts_on"
        ...
        <fieldset className="course-online-only gap-5 border-t border-line pt-5">
          ...
          <Field
            label="Нэмэлт заавар"
            hint="Элсэгчид Telegram товчны доор харагдана"
          >
            <Textarea
              name="access_note_mn"
              rows={2}
              defaultValue={access?.note_mn ?? ""}
            />
          </Field>
        </fieldset>
          ...
          <div className="flex flex-col gap-5 border-t border-line p-4">
            ...
            <Field label="Description (EN)">
              <Textarea
                name="desc_en"
                rows={4}
                defaultValue={course?.desc_en ?? ""}
              />
            </Field>
            <Field label="Schedule (EN)">
              <Input
                name="schedule_en"
                defaultValue={course?.schedule_en ?? ""}
              />
            </Field>
        ...
        <form action={deleteCourse} className="mt-5 border-t border-line pt-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="t-meta text-muted">
              Элсэгчгүй тул устгаж болно. Элсэгчтэй болсны дараа зөвхөн идэвхгүй
              болгоно.
            </p>
